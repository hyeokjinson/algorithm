def go(n,cnt):
    ...
    for _ in range(cnt):
        ...
        if x<0 or x>=B or y<0 or y>=A:
            ...
            if board[x][y]:
                print("Robot {} crashes into robot {}".format(n,board[x][y]))
                return False
    robot[n][0],robot[n][1]=x,y
    board[x][y]=n
...
if __name__ == '__main__':
    ...
    for num in range(1,n+1):
        ...
        if direction=='N':d=0
        elif direction=='E':d=1
        elif direction=='S':d=2
        elif direction=='W':d=3
        robot.append([x,y,d])
    flag=True
    for _ in range(m):
        name,c,cnt=input().split()
        if flag:
            flag=excute(int(name),c,int(cnt))
